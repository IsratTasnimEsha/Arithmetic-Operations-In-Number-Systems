...
bool valid(st main_number){
    ...
    if(number_system=="bin") last_symbol='1';
...
dl to_decimal(st main_number)
{
    ...
    if(flag) point=main_number.size();
    f(0, i, point){
        if(main_number[i]>='0' && main_number[i]<='9') converted_int=main_number[i]-'0';
        else{
    ...
    f(point+1, i, main_number.size()){
        k--;
        if(main_number[i]>='0' && main_number[i]<='9') converted_flt=main_number[i]-'0';
        else{
            extra=(ll)main_number[i]-64;
...
int main()
{
    ...
    bye;
    ct("\t- addition of several binary numbers"<<endl<<"\t- addition of several octal numbers"<<endl);
    ct("\t- addition of several decimal numbers"<<endl<<"\t- addition of several hexa-decimal numbers"<<endl);
    ct("\t- subtraction of two binary numbers"<<endl<<"\t- subtraction of two octal numbers"<<endl);
    ct("\t- subtraction of two decimal numbers"<<endl<<"\t- subtraction of two hexa-decimal numbers"<<endl);
    ct("\t- multiplication of several binary numbers"<<endl<<"\t- multiplication of several octal numbers"<<endl);
    ct("\t- multiplication of several decimal numbers"<<endl<<"\t- multiplication of several hexa-decimal numbers"<<endl);
    ct("\t- division of two binary numbers"<<endl<<"\t- division of two octal numbers"<<endl);
    ct("\t- division of two decimal numbers"<<endl<<"\t- division of two hexa-decimal numbers");
    bye;
    ct("(Both integer and float numbers are acceptable.)");
    bye, bye;
    ct("#Use add for addition, sub for subtraction, mul for multiplication, div for division, bin for binary, oct for octal, dec for decimal and hex for hexa-decimal.");
    bye, bye;
    ll t=0;
    while(1){
        t++;
            ...
            if(operation=="add" || operation=="sub" || operation=="mul" || operation=="div"){
                if(operation=="add" || operation=="mul"){
                    ...
                    f(1, i, time+1){
                        ...
                        cst(temp);
                        if(valid(temp)){
                            if(operation=="add") sum+=to_decimal(temp);
                            else mul*=to_decimal(temp);
                        }
                        else{
                            Valid=false;
                            break;
                    ...
                    if(Valid){
                        if(operation=="add") answer=to_string(sum);
                        else answer=to_string(mul);
                        ct("Ans: ");
                        from_decimal(answer);
                    }
                }
                    ...
                    if(valid(number1)){
                        ...
                        if(valid(number2)){
                            ...
                            if(operation=="div"){
                                ...
                                if(Valid) ans=to_decimal(number1)/to_decimal(number2);
                            }
                            if(Valid){
